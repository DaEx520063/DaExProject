...
def api_salary_monthly_data():
    ...
    try:
        ...
        for data in salary_data:
            ...
            # คำนวณข้อมูลช่วงน้ำหนักใหม่ 12 ช่วงจาก employee_salary_records ตามเดือน/ปี
            work_month = f"{int(year):04d}-{int(month):02d}"
            cursor.execute('''
                SELECT 
                    SUM(CASE WHEN weight <= 0.5 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 0.5 AND weight <= 1.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 1.0 AND weight <= 1.5 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 1.5 AND weight <= 2.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 2.0 AND weight <= 2.5 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 2.5 AND weight <= 3.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 3.0 AND weight <= 4.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 4.0 AND weight <= 5.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 5.0 AND weight <= 10.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 10.0 AND weight <= 15.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 15.0 AND weight <= 30.0 THEN 1 ELSE 0 END),
                    SUM(CASE WHEN weight > 30.0 THEN 1 ELSE 0 END)
                FROM employee_salary_records 
                WHERE employee_id = ? AND work_month = ?
            ''', (emp_id, work_month))
            range_counts = cursor.fetchone() or [0]*12
            range_names = [
                '0.00-0.50KG','0.51-1.00KG','1.01-1.50KG','1.51-2.00KG','2.01-2.50KG','2.51-3.00KG',
                '3.01-4.00KG','4.01-5.00KG','5.01-10.00KG','10.01-15.00KG','15.01-30.00KG','30.01KG+'
            ]
            weight_ranges = [{ 'range': rn, 'pieces': (range_counts[i] or 0), 'amount': 0 } for i, rn in enumerate(range_names)]
...
def api_employee_salary_details(employee_id):
    ...
    try:
        ...
        # สร้างข้อมูลช่วงน้ำหนักใหม่ 12 ช่วงจากตาราง employee_salary_records ตามเดือนล่าสุดของพนักงานนี้
        cursor.execute('''
            SELECT work_month FROM employee_salary_records 
            WHERE employee_id = ?
            ORDER BY work_month DESC
            LIMIT 1
        ''', (employee_id,))
        ...
        cursor.execute('''
            ...
            FROM employee_salary_records 
            WHERE employee_id = ? AND work_month = ?
        ''', (employee_id, work_month))
        counts = cursor.fetchone() or [0]*12
        ...
        for i, rn in enumerate(range_names):
            count = counts[i] or 0
            weight_ranges.append({'range': rn, 'count': count, 'rate': 0, 'amount': 0})
            total_pieces += count
    ...
    try:
        ...
        cursor.execute('''
            SELECT 
                ...
                SUM(CASE WHEN weight > 3.0 AND weight <= 4.0 THEN 1 ELSE 0 END) as range_7,
                SUM(CASE WHEN weight > 4.0 AND weight <= 5.0 THEN 1 ELSE 0 END) as range_8,
                SUM(CASE WHEN weight > 5.0 AND weight <= 10.0 THEN 1 ELSE 0 END) as range_9,
                SUM(CASE WHEN weight > 10.0 AND weight <= 15.0 THEN 1 ELSE 0 END) as range_10,
                SUM(CASE WHEN weight > 15.0 AND weight <= 30.0 THEN 1 ELSE 0 END) as range_11,
                SUM(CASE WHEN weight > 30.0 THEN 1 ELSE 0 END) as range_12
        ...
        for row in cursor.fetchall():
            emp_id, name, total_packages, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12 = row
            ...
            employees.append({
                ...
                'weight_ranges': {
                    ...
                    '3.01-4.00KG': r7 or 0,
                    '4.01-5.00KG': r8 or 0,
                    '5.01-10.00KG': r9 or 0,
                    '10.01-15.00KG': r10 or 0,
                    '15.01-30.00KG': r11 or 0,
                    '30.01KG+': r12 or 0
                }
            })
        ...
        cursor.execute('''
            SELECT 
                CASE 
                    ...
                    WHEN weight <= 4.0 THEN '3.01-4.00KG'
                    WHEN weight <= 5.0 THEN '4.01-5.00KG'
                    WHEN weight <= 10.0 THEN '5.01-10.00KG'
                    WHEN weight <= 15.0 THEN '10.01-15.00KG'
                    WHEN weight <= 30.0 THEN '15.01-30.00KG'
                    ELSE '30.01KG+'
                END as weight_range,
    ...
    try:
        ...
        activities = [
            ...
            },
            {
                ...
                'description': 'Uploaded salary data'
            },
            {
...
def api_vehicle_check():
    ...
    try:
        ...
        if not inspector_id:
            return jsonify({'success': False, 'message': 'กรุณาเข้าสู่ระบบอีกครั้ง'}), 401

        vehicle_id = request.form.get('vehicle_id')
        check_date = request.form.get('check_date')

        if not vehicle_id or not check_date:
        ...
        engine_status_old = request.form.get('engine_status', '')
        engine_notes_old = request.form.get('engine_notes', '')
        body_status_old = request.form.get('body_status', '')
        body_notes_old = request.form.get('body_notes', '')
        brakes_status_old = request.form.get('brakes_status', '')
        brakes_notes_old = request.form.get('brakes_notes', '')
        interior_status_old = request.form.get('interior_status', '')
        interior_notes_old = request.form.get('interior_notes', '')
        ...
        for form_field, db_field in old_image_fields_map.items():
            if db_field not in image_data or image_data[db_field] is None:
                ...
                if file and file.filename:
                    ...
                    image_data[db_field] = image_data.get(db_field)
        ...
        engine_status_value = engine_status_old or oil_status
        engine_notes_value = engine_notes_old or oil_notes
        body_status_value = body_status_old or doors_status
        body_notes_value = body_notes_old or doors_notes
        brakes_status_value = brakes_status_old or brake_fluid_status
        brakes_notes_value = brakes_notes_old or brake_fluid_notes
        interior_status_value = interior_status_old or mirrors_status
        interior_notes_value = interior_notes_old or mirrors_notes
        ...
        values = [
            ...
            engine_status_value, engine_notes_value, image_data.get('engine_image'),
            body_status_value, body_notes_value, image_data.get('body_image'),
            brakes_status_value, brakes_notes_value, image_data.get('brakes_image'),
            interior_status_value, interior_notes_value, image_data.get('interior_image'),
...
def api_export_check_history():
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            for row_idx, images in enumerate(images_data, start=2):
